...
class FeaturedModel {
    ...
    function getDestacadas(){
        $sentencia = $this->db->prepare('SELECT destacadas.*, ciudades.nombre as ciudad FROM destacadas INNER JOIN ciudades ON destacadas.ciudad = ciudades.idCiudad');
        $sentencia->execute();
    ...
    function getDestacadasFiltradasOrdenadas($operacion, $sort, $order){
        $sentencia = $this->db->prepare("SELECT destacadas.*, ciudades.nombre as ciudad FROM destacadas INNER JOIN ciudades ON destacadas.ciudad = ciudades.idCiudad WHERE operacion=? ORDER BY $sort $order");
        $sentencia->execute(array($operacion));
        $destacadas = $sentencia->fetchAll(PDO::FETCH_OBJ);
        return $destacadas;
    } 
    function getDestacadasFiltradasOrdenadasPaginadas($operacion, $sort, $order, $limit, $offset){
        $sentencia = $this->db->prepare("SELECT destacadas.*, ciudades.nombre as ciudad FROM destacadas INNER JOIN ciudades ON destacadas.ciudad = ciudades.idCiudad WHERE operacion=? ORDER BY $sort $order LIMIT $limit OFFSET $offset");
        $sentencia->execute(array($operacion));
        $destacadas = $sentencia->fetchAll(PDO::FETCH_OBJ);
        return $destacadas;
    } 
    ...
    function getDestacadasPaginadas($limit, $offset){
        $sentencia = $this->db->prepare("SELECT destacadas.*, ciudades.nombre as ciudad FROM destacadas INNER JOIN ciudades ON destacadas.ciudad = ciudades.idCiudad LIMIT $limit OFFSET $offset");
        $sentencia->execute();
        $destacadas = $sentencia->fetchAll(PDO::FETCH_OBJ);
        return $destacadas;
    } 
    ...
    function getDestacada($id){
        $sentencia = $this->db->prepare('SELECT destacadas.*, ciudades.nombre as ciudad FROM destacadas INNER JOIN ciudades ON destacadas.ciudad = ciudades.idCiudad WHERE idDestacada=?');
